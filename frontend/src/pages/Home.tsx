...
export const Home: React.FC = () => {
  ...
  const filteredDestinations = sampleDestinations.filter((dest) => {
    ...
    const matchesSearch =
      dest.title.toLowerCase().includes(searchLocation.toLowerCase()) ||
      dest.country.toLowerCase().includes(searchLocation.toLowerCase());
    ...
    <div className="bg-white dark:bg-dark text-slate-900 dark:text-light overflow-x-hidden font-sans selection:bg-primary selection:text-white transition-colors duration-300">
      ...
      <section className="relative h-screen w-full flex items-center justify-center overflow-hidden">
        ...
        <div className="relative max-w-5xl mx-auto px-6 text-center z-20 mt-12">
          <motion.div
            ...
            className="inline-flex items-center space-x-2 bg-white/20 backdrop-blur-md px-4 py-1.5 rounded-full border border-white/30 mb-6 text-white text-xs font-semibold tracking-wider uppercase"
          >
          ...
          <motion.p
            ...
            className="text-base md:text-xl text-white/90 max-w-2xl mx-auto mb-12 font-medium"
          >
            Discover unforgettable premium adventures and customized dream
            destinations designed exclusively for you.
          </motion.p>
          ...
          >
            ...
            <div className="flex flex-col space-y-1.5 p-2 border-t md:border-t-0 md:border-l border-white/10">
              ...
              <input
                type="date"
                className="bg-transparent border-none outline-none text-white text-sm w-full font-semibold filter brightness-200"
              />
            </div>
          ...
          {[
            {
              ...
              desc: "Every booking contains luxury insurance coverage and fully certified safety protocols.",
            },
            {
              icon: <Award className="w-8 h-8 text-secondary" />,
              title: "Award Winning Guide",
              desc: "Ranked top luxury travel management system by prestigious world tourism metrics.",
            },
            {
              icon: <Users className="w-8 h-8 text-accent" />,
              title: "Elite Community",
              desc: "Join over 50,000+ VIP travelers creating shared moments worldwide.",
            },
          ].map((feat, idx) => (
            <motion.div
              key={idx}
              ...
              whileInView="visible"
              viewport={{ once: true }}
              className="bg-slate-50 dark:bg-dark-light border border-slate-100 dark:border-white/5 p-8 rounded-3xl shadow-xs hover:shadow-xl transition-all duration-300 group"
            >
      ...
      <section className="py-12 max-w-7xl mx-auto px-6">
        <div className="flex flex-col md:flex-row justify-between items-start md:items-end mb-12 gap-6">
          <div>
            ...
            <h2 className="text-4xl md:text-5xl font-bold font-display tracking-tight text-slate-900 dark:text-white">
            ...
            {filteredDestinations.map((dest) => (
              ...
              >
                <div className="relative h-72 overflow-hidden">
                  ...
                  >
                    <Heart
                      className={`w-5 h-5 transition-colors ${favorites.includes(dest.id) ? "fill-accent text-accent" : "text-slate-700 dark:text-light"}`}
                    />
                  </button>
                ...
                <div className="p-6 relative bg-white dark:bg-dark-light">
                  <span className="text-xs text-primary font-extrabold tracking-wider uppercase mb-1.5 block">
                    {dest.country}
                  </span>
                  ...
                  <div className="flex justify-between items-center border-t border-slate-100 dark:border-white/5 pt-4">
                    ...
                    <span className="text-2xl font-extrabold text-transparent bg-clip-text bg-linear-to-r from-primary to-secondary">
                      ${dest.price}
                    </span>
                  </div>
                </div>
        ...
        {filteredDestinations.length === 0 && (
          <div className="text-center py-12 font-medium text-slate-400">
            No destinations found matching your active search criteria.
          </div>
        )}
      </section>
      ...
      <section className="py-16 max-w-7xl mx-auto px-6">
        <div className="text-center mb-16">
          ...
          <h2 className="text-4xl md:text-5xl font-bold font-display mt-2 text-slate-900 dark:text-white">
            Shared Moments
          </h2>
        </div>
          ...
          {[
            ...
            >
              ...
              <div className="absolute inset-0 bg-black/50 opacity-0 group-hover:opacity-100 transition-all duration-300 flex items-center justify-center">
                <span className="text-white text-xs font-bold tracking-wider uppercase bg-primary px-4 py-2 rounded-full flex items-center gap-1">
                  <Eye className="w-3.5 h-3.5" /> View Escape
                </span>
              </div>
            </motion.div>
          ))}
      ...
      <section className="py-16 max-w-7xl mx-auto px-6 mb-12">
        <div className="bg-linear-to-br from-primary/10 via-secondary/5 to-transparent dark:from-white/5 dark:to-transparent border border-primary/20 dark:border-white/10 rounded-[3rem] p-8 md:p-14 flex flex-col md:flex-row items-center justify-between gap-8 relative overflow-hidden">
          ...
          <div className="max-w-xl text-center md:text-left space-y-4">
            ...
            <h3 className="text-3xl md:text-4xl font-bold font-display tracking-tight text-slate-900 dark:text-white">
              Get 20% Off Your First Luxury Voyage
            </h3>
            <p className="text-slate-500 dark:text-gray-400 text-sm md:text-base leading-relaxed">
              Sign up to our VIP mailing pipeline today and access unlisted
              5-star packages across Europe and Southeast Asia.
            </p>
          </div>
